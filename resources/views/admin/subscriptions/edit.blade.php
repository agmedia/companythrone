    ...
    <div class="container">
        <form action="{{ route('subscriptions.update', $subscription) }}" method="POST" class="card">
            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('subscriptions.show', $subscription) }}" class="btn btn-light">Back</a>
            </div>
                ...
                <div class="col-md-2">
                    ...
                    <select name="period" class="form-select @error('period') is-invalid @enderror">
                        @foreach(['monthly','yearly'] as $p)
                            <option value="{{ $p }}" @selected(old('period', $subscription->period) === $p)>{{ ucfirst($p) }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror">
                        @foreach($statuses as $st)
                            <option value="{{ $st }}" @selected(old('status', $subscription->status) === $st)>{{ ucfirst($st) }}</option>
                        @endforeach
                    </select>
