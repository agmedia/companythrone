...
@section('content')
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb mb-0">
            ...
            @foreach($breadcrumbs as $crumb)
                ...
                @else
                    <li class="breadcrumb-item">
                        <a href="{{ localized_route('categories.show', ['category' => $crumb]) }}">{{ $crumb->name }}</a>
                    </li>
                @endif
            @endforeach
        </ol>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <span class="badge text-bg-primary">{{ trans_choice('category.company_count', $companies->total(), ['count'=>$companies->total()]) }}</span>
    </div>
                ...
                <div class="d-flex flex-wrap gap-2">
                    @foreach($children as $child)
                        <a class="badge text-bg-light text-decoration-none"
                           href="{{ localized_route('categories.show', ['category' => $child]) }}">
                            {{ $child->name }}
                        </a>
                    @endforeach
                </div>
